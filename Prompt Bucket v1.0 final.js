...
// Prompt Bucket with Copyable Text Box
// Prompt Bucket with Text Box Save
// Prompt generation logic remains the same
// Advanced Cinematic Prompt Generator for FLUX.1 and similar models

// generate a pic on each press 


//grab the current configuration
const configuration = pipeline.configuration;
const runMask = canvas.currentMask;



// Core wordbanks for prompt generation
...
//  ],
  ...
  materialsTextures: [
    ...
    "fractured glass", "cracked leather", "weathered wood", "eroded stone", "corroded metal"
  ],
  ...
  // Weighted components will create more emphasis
  getWeightedPhrase: function(text, importance) {
    if (importance >= 0.8) {
      // Very important terms get double parentheses
      const weightIndex = Math.floor(Math.random() * 2) + 1; // (( or ((( 
      return this.weightModifiers[weightIndex] + text + this.weightModifiers[6 - weightIndex];
    } else if (importance >= 0.5) {
      // Important terms get single parentheses
      return this.weightModifiers[0] + text + this.weightModifiers[6];
    } else if (importance >= 0.3) {
      // Somewhat important terms get colons
      const weightIndex = Math.floor(Math.random() * 3) + 7; // : or :: or :::
      return text + this.weightModifiers[weightIndex];
    }
    return text; // Normal importance, no weighting
  },
  ...
  // Create narrative movie-like descriptions
  createNarrativeDescription: function() {
    // Scene setting components
    const subject = this.getRandomItem(this.subjects);
    const mood = this.getRandomItem(this.moods);
    const timePeriod = this.getRandomItem(this.timePeriods);
    const colorPalette = this.getRandomItem(this.colorPalettes);
    const sceneDescriptor = this.getRandomItem(this.sceneDescriptors);
    
    // Character components
    const character = this.getRandomItem(this.characters);
    
    // Technical components
    const renderingTerms = this.getRandomItems(this.renderingTerms, Math.floor(Math.random() * 4) + 2);
    const cinematicTerms = this.getRandomItems(this.cinematicTerms, Math.floor(Math.random() * 4) + 2);
    const lightingTerms = this.getRandomItems(this.lightingTerms, Math.floor(Math.random() * 3) + 2);
    const gameGraphicsTerms = this.getRandomItems(this.gameGraphicsTerms, Math.floor(Math.random() * 4) + 2);
    const technicalDetails = this.getRandomItems(this.technicalDetails, Math.floor(Math.random() * 3) + 1);
    
    // Stylistic components
    const artisticStyle = this.getRandomItem(this.artisticStyles);
    const cameraSettings = this.getRandomItems(this.cameraSettings, Math.floor(Math.random() * 2) + 1);
    const director = this.getRandomItem(this.directors);
    const cinematographyTech = this.getRandomItems(this.cinematographyTechniques, Math.floor(Math.random() * 3) + 1);
    const weatherCondition = this.getRandomItem(this.weather);
    const materials = this.getRandomItems(this.materialsTextures, Math.floor(Math.random() * 3) + 1);
    const specialEffect = this.getRandomItems(this.specialEffects, Math.floor(Math.random() * 3) + 1);

    // Combine everything into a coherent scene description
    
    const prompt = `In a ${mood}, ${timePeriod} setting, a ${character} appears in a ${subject}, surrounded by ${colorPalette}. The moment captures ${sceneDescriptor}, featuring ${renderingTerms.join(", ")}, ${cinematicTerms.join(", ")}, and ${lightingTerms.join(", ")}. Technical details include ${gameGraphicsTerms.join(", ")}, ${technicalDetails.join(", ")}. Shot with ${cameraSettings.join(", ")}, composed using ${cinematographyTech.join(", ")} techniques under ${weatherCondition}. The style leans toward ${artisticStyle}, influenced by ${director}. The environment includes ${materials.join(", ")}, enhanced with ${specialEffect.join(", ")}.`;
...
while (true) {
// Generate the prompt
let promptGenerated = promptGenerator.createNarrativeDescription();

// Show Create screen with text box that saves the generated prompt
requestFromUser("Prompt Bucket", "Create", function() {
  return [
    this.plainText("Click Create to generate a cinematic prompt."),
    this.textField(promptGenerated, "Generated Prompt", true, 320)
  ];
});
...
//generational part!
pipeline.run({
   configuration: configuration, //  got at the beginning ,
   prompt: promptGenerated, // // Use modifiedPromptz here
   mask: runMask
 	 });
...
console.repl(); // with while(true) loop it will never be executed

//
// I do not what was hallucinated below
// Also log the prompt

//// Display prompt in a selectable text field for manual copy
//requestFromUser("Prompt Bucket", "Create", function() {
//  return [
//    this.plainText("Click Create to generate a cinematic prompt."),
//    this.textField(prompt, "Copy the Prompt Below", true, 320)
//  ];
//});
